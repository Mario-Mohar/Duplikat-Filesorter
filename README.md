...
Während des Laufs zeigt die App Fortschritt, Anzahl gescannter Dateien, gefundener Duplikate und den betroffenen Speicherplatz. **Abbrechen** stoppt sauber zwischen zwei Dateien, bereits verschobene Dateien bleiben verschoben und stehen im Log.
...
- **Verschoben wird per `rename`.** Das ist sofort fertig, funktioniert aber nur innerhalb desselben Dateisystems. Liegt der Duplikate-Ordner auf einer anderen Platte, schlägt das Verschieben fehl, die Datei bleibt liegen und der Fehler steht im Log.
- **Symlinks werden verfolgt.** Ein Ordner, der auf sich selbst zeigt, kann die Suche in eine Schleife schicken.
- **Gehasht wird der Reihe nach**, nicht parallel. Bei sehr vielen gleich großen, sehr großen Dateien merkt man das.
